...
export default function EditExpenseModal({
  ...
  const [description, setDescription] = useState(expense.description || expense.label || "");
  const [category, setCategory] = useState(expense.category || "OTHER");
  const [amount, setAmount] = useState(
    expense.amount == null ? "" : String(expense.amount),
  );
  const [currency, setCurrency] = useState(expense.currency || trip?.currency || "USD");
  const [expenseDate, setExpenseDate] = useState(
    expense.expenseDate || expense.incurredOn || trip?.startDate || "",
  );
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setErrors({
        ...
        form: explainApiError(err, "Unable to update this expense"),
      });
    }
  }
